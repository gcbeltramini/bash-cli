...
_mycli_extract_arguments_with_descriptions() {
  ...
  # # TODO: create function with input: 'docopt_options'; output 'args_description'
  # local description line
  # local args_description=""
  # local -r description_fallback="<no description>"
  # # TODO: create function with input 'docopt_options'; output 'options_without_descriptions'
  # local -r options_without_descriptions=$(echo "$docopt_options" | sed -E 's/^[[:space:]]*// ; s/ {2,}.*//')
  # while IFS= read -r arg; do
  #   # TODO: create function with input: 'arg', 'docopt_options', 'options_without_descriptions'; output 'description' (can be empty; treat fallback outside)
  #   if line_number=$(grep -nF -- "$arg" <<< "$options_without_descriptions" | cut -d: -f1); then
  #     description=$(sed -n "${line_number}p" <<< "$docopt_options" |
  #       sed 's/^[[:space:]]*//' |
  #       grep -Eo ' {2,}.*' |
  #       sed 's/^[[:space:]]*//' || echo "$description_fallback")
  #   else
  #     description=$description_fallback
  #   fi
  #   args_description+="$arg:$description\n"
  # done <<< "$args"

  local -r args_description=$(_mycli_get_args_description "$args" "$docopt_options")

  echo -e "$args_description\n$help_description" | sed '/^[[:space:]]*$/d'
}
...
_mycli_completions() {
  ...
  # Case 3: The user is completing the parameters or other commands
    ...
    # local -r all_args="$(_extract_arguments "${words[2]}" "$words[3]")"
    ...
    # local -r args=$(grep -Fxv -f <(printf '%s\n' "${words[@]}") <<< "$all_args")
    ...
    # Convert the multiline string to an array
    # local -ra args_array=("${(f)args}")
    local -ra args_with_description_array=("${(f)args_with_description}")

    # _describe 'mycli parameters' args_array
